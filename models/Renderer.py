...
class voxel_renderer:
    def __init__(self, render_IO_vox_size=256, render_boundary_padding_size=16):
        ...
        self.render_fix_vox_size = (
            self.render_IO_vox_size + self.render_boundary_padding_size * 2
        )
        self.voxel_idxs = np.linspace(
            -self.render_fix_vox_size / 2 + 0.5,
            self.render_fix_vox_size / 2 - 0.5,
            self.render_fix_vox_size,
            dtype=np.float32,
        )
    ...
    def render_img(
        ...
        if self.render_IO_vox_size != imgsize:
            print(
                "ERROR: render_img() voxel size does not match!",
                imgsize,
                self.render_IO_vox_size,
            )
            exit(-1)
        ...
        # right-top
        if view == 1:
            new_x = (-self.voxel_x + self.voxel_y) / 2**0.5 + (
                self.render_fix_vox_size / 2 + 0.01
            )
            new_y = (self.voxel_x + self.voxel_y) / 2**0.5 + (
                self.render_fix_vox_size / 2 + 0.01
            )
        ...
        if view == 2:
            ...
            new_y = (-self.voxel_y + self.voxel_z) / 2**0.5 + (
                self.render_fix_vox_size / 2 + 0.01
            )
            new_z = (self.voxel_y + self.voxel_z) / 2**0.5 + (
                self.render_fix_vox_size / 2 + 0.01
            )
            new_x = np.clip(new_x.astype(np.int32), 0, self.render_fix_vox_size - 1)
            new_y = np.clip(new_y.astype(np.int32), 0, self.render_fix_vox_size - 1)
        ...
        # left-front
        if view == 3:
            new_x = (self.voxel_x + self.voxel_z) / 2**0.5 + (
                self.render_fix_vox_size / 2 + 0.01
            )
            new_y = self.voxel_y + (self.render_fix_vox_size / 2 + 0.01)
            new_z = (self.voxel_x - self.voxel_z) / 2**0.5 + (
                self.render_fix_vox_size / 2 + 0.01
            )
            new_x = np.clip(new_x.astype(np.int32), 0, self.render_fix_vox_size - 1)
            new_y = np.clip(new_y.astype(np.int32), 0, self.render_fix_vox_size - 1)
        ...
        # surface
            ...
            output = np.ones(
                [self.render_fix_vox_size, self.render_fix_vox_size], np.float32
            )
            dx = depth[:, :-1] - depth[:, 1:]
            dy = depth[:-1, :] - depth[1:, :]
            dxp = 0
            ...
            for iii in range(12):
                if iii == 0:  # /\ 12 25
                    ...
                    partial_dx = dx[1:-1, :-1]
                    partial_dy = dy[1:, :-2]
                partial_m = (np.abs(partial_dx) < steep_threshold) & (
                    np.abs(partial_dy) < steep_threshold
                )
    ...
    def render_img_with_camera_pose(
        ...
        if get_depth:  # depth
            ...
            output = output[
                self.render_boundary_padding_size
                - 1 : -self.render_boundary_padding_size
                + 1,
                self.render_boundary_padding_size
                - 1 : -self.render_boundary_padding_size
                + 1,
    ...
    def use_gpu(self):
        ...
        self.voxel_x_tensor = torch.from_numpy(
            self.voxel_x / (self.render_fix_vox_size / 2)
        ).to(self.device)
        self.voxel_y_tensor = torch.from_numpy(
            self.voxel_y / (self.render_fix_vox_size / 2)
        ).to(self.device)
        self.voxel_z_tensor = torch.from_numpy(
            self.voxel_z / (self.render_fix_vox_size / 2)
        ).to(self.device)
    ...
    def render_img_with_camera_pose_gpu(
        ...
        new_xyz_tensor = torch.cat(
            [
                new_x_tensor.unsqueeze(3),
                new_y_tensor.unsqueeze(3),
                new_z_tensor.unsqueeze(3),
            ],
            3,
        ).unsqueeze(0)

        voxel_tensor = F.grid_sample(
            voxel_tensor, new_xyz_tensor, mode="bilinear", padding_mode="zeros"
        ).squeeze()
        ...
        if get_depth:  # depth
            ...
            for iii in range(12):
                if iii == 0:  # /\ 12 25
                    ...
                    partial_dx = dx[1:-1, :-1]
                    partial_dy = dy[1:, :-2]
                partial_m = (torch.abs(partial_dx) < steep_threshold) & (
                    torch.abs(partial_dy) < steep_threshold
                )
