...
class ShaddrDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        voxel_data = F.max_pool3d(torch.tensor(voxel_data, dtype=torch.float32).unsqueeze(0), 
                                 downsample_size, downsample_size).squeeze(0).numpy().astype(np.uint8)
        ...
        if self.is_style == True:
            ...
            voxels_sm = F.max_pool3d(torch.from_numpy(voxels.astype(np.float32)).unsqueeze(0).unsqueeze(0), 
                                    kernel_size=2, stride=2, padding=0).numpy()[0, 0]
            ...
            back, front, top, left, right = self.get_rendered_views(hdf5_file, xmin, xmax, ymin, ymax, zmin, zmax)
            render_style = [back, front, top, left]

            net_input = {"mask_g": gmask_style,
            ...
            input_content, dmask_content_lg, dmask_content_sm, gmask_content = self.get_voxel_input_Dmask_Gmask(voxels)
            pos_content = [xmin, xmax, ymin, ymax, zmin, zmax]

            net_input = {"mask_g": gmask_content,
